"""
Created on May 23 2026

@author: Simão Ribeiro

This script builds a Radial Basis Function (RBF) regression model to predict
wing download force [N] as a function of horizontal (d/D) and vertical (z/D)
distances between an actuator disk and a wing. The model is trained on 9 CFD
simulation points and uses hyperparameter tuning with cross-validation.

Methodological traceability:
- Design matrix includes: constant term + linear terms (d/D, z/D) + RBF kernels.
- Three kernel types: Gaussian, Matérn C4, compact C4.
- Hyperparameter sweep over sigma and alpha.
- 3-fold cross-validation (shuffled, fixed seed) for RMSE estimation.
- Best model selected by lowest average RMSE; final model retrained on all data.
- Condition number of Hessian matrix is monitored for numerical stability.
- MinMax scaling applied to inputs before building the design matrix.
"""

import numpy as np
import matplotlib.pyplot as plt
from scipy.spatial.distance import cdist
from sklearn.preprocessing import MinMaxScaler
from sklearn.linear_model import Ridge
from sklearn.model_selection import KFold
from sklearn.metrics import mean_squared_error
from typing import Tuple, List

#%% 1. LOAD CFD DATA

def load_cfd_data() -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Load the CFD simulation data for eVTOL wing download force.

    Returns
    -------
    dD : np.ndarray, shape (9,)
        Nondimensional horizontal distances d/D.
    zD : np.ndarray, shape (9,)
        Nondimensional vertical distances z/D.
    force : np.ndarray, shape (9,)
        Wing download force [N] from CFD.
    """
    dD = np.array([0.25, 0.25, 0.25, 0.375, 0.375, 0.375, 0.50, 0.50, 0.50])
    zD = np.array([0.50, 0.75, 1.00, 0.50, 0.75, 1.00, 0.50, 0.75, 1.00])
    force = np.array([-9.5, -9.8, -11.0, -2.4, -3.7, -4.7, -1.3, -1.5, -1.5])
    return dD, zD, force

#%% 2. BUILD DESIGN MATRIX

def build_design_matrix(X: np.ndarray, centers: np.ndarray, sigma: float, kernel: str) -> np.ndarray:
    """
    Build the design matrix Phi = [phi1, phi2, ..., phin_centers].

    Parameters
    ----------
    X : np.ndarray, shape (n_samples, 2)
        Scaled input coordinates.
    centers : np.ndarray, shape (n_centers, 2)
        RBF centre positions (also scaled).
    sigma : float
        RBF length scale.
    kernel : {'Gaussian', 'Matérn', 'Compact'}
        Type of RBF kernel.

    Returns
    -------
    Phi : np.ndarray, shape (n_samples, n_centers)
        Design matrix where Phi[i, j] = phi_j(X[i]).

    Raises
    ------
    ValueError
        If kernel type is unknown.
    """
    r = cdist(X, centers, metric='euclidean')   # shape (n_samples, n_centers)
    
    if kernel == 'Gaussian':
        return np.exp(-(r ** 2) / (sigma ** 2))
    elif kernel == 'Matérn':
        return np.exp(-r / sigma) * ((r ** 2) / (sigma ** 2) + 3 * r / sigma + 3)
    elif kernel == 'Compact':
        return np.where(r <= sigma, ((1 + r / sigma) ** 5) * ((1 - r / sigma) ** 5), 0.0)
    else:
        raise ValueError(f"Unknown kernel: {kernel}")
    
#%% 3. HYPERPARAMETER TUNING USING K-FOLD CV

def grid_search_rbf(
    X_scaled: np.ndarray,
    y: np.ndarray,
    centers: np.ndarray,
    kernels: List[str],
    sigmas: np.ndarray,
    alphas: np.ndarray,
    n_folds: int = 3,
    random_seed: int = 1
) -> Tuple[float, float, str, Ridge, float]:
    """
    Perform grid search over kernels, sigma, alpha using K-fold cross-validation.

    Parameters
    ----------
    X_scaled : np.ndarray, shape (n_samples, 2)
        Scaled input features.
    y : np.ndarray, shape (n_samples,)
        Target values (force).
    centers : np.ndarray, shape (n_centers, 2)
        RBF centres (scaled).
    kernels : List[str]
        List of kernel names to test.
    sigmas : np.ndarray
        Array of sigma values to test.
    alphas : np.ndarray
        Array of alpha (Ridge regularisation) values to test.
    n_folds : int, default=3
        Number of folds for cross-validation.
    random_seed : int, default=1
        Seed for KFold shuffling.

    Returns
    -------
    best_rmse : float
        Lowest average RMSE across folds.
    best_sigma : float
        Corresponding sigma.
    best_alpha : float
        Corresponding alpha.
    best_kernel : str
        Corresponding kernel name.
    best_model : Ridge
        Trained Ridge model on all data with best hyperparameters.
    best_condition : float
        Condition number of H = Phi^T @ Phi for the best configuration.
    """
    kfold = KFold(n_splits=n_folds, shuffle=True, random_state=random_seed)
    best_rmse = np.inf
    best_sigma = None
    best_alpha = None
    best_kernel = None
    best_model = None
    best_condition = None

    for kernel in kernels:
        for sigma in sigmas:
            for alpha in alphas:
                # Build design matrix
                Phi = build_design_matrix(X_scaled, centers, sigma, kernel)

                # H
                H_reg = Phi.T @ Phi + alpha * np.eye(Phi.shape[1])
                cond_H = np.linalg.cond(H_reg)
                
                # Security
                if np.isnan(cond_H) or np.isinf(cond_H) or cond_H > 1e6:
                    continue

                # Cross-validation
                errors = []
                for train_idx, test_idx in kfold.split(Phi):
                    Phi_train = Phi[train_idx]
                    Phi_test = Phi[test_idx]
                    y_train = y[train_idx]
                    y_test = y[test_idx]

                    model_cv = Ridge(alpha=alpha, fit_intercept=False)
                    model_cv.fit(Phi_train, y_train)
                    y_pred = model_cv.predict(Phi_test)
                    rmse = np.sqrt(mean_squared_error(y_test, y_pred))
                    errors.append(rmse)

                avg_rmse = np.mean(errors)

                if avg_rmse < best_rmse:
                    best_rmse = avg_rmse
                    best_sigma = sigma
                    best_alpha = alpha
                    best_kernel = kernel
                    best_condition = cond_H
                    best_model = Ridge(alpha=alpha, fit_intercept=False)
                    best_model.fit(Phi, y)

    return best_rmse, best_sigma, best_alpha, best_kernel, best_model, best_condition

#%% 4. PREDICTION ON GRID AND VISUALISATION

def create_prediction_grid(
    d_range: Tuple[float, float] = (0.25, 0.50),
    z_range: Tuple[float, float] = (0.50, 1.00),
    n_points: int = 100
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Create a regular 2D grid of points for prediction.

    Parameters
    ----------
    d_range : tuple (d_min, d_max)
        Horizontal distance range.
    z_range : tuple (z_min, z_max)
        Vertical distance range.
    n_points : int
        Number of points along each dimension.

    Returns
    -------
    D_grid : np.ndarray, shape (n_points, n_points)
        2D grid of horizontal distances.
    Z_grid : np.ndarray, shape (n_points, n_points)
        2D grid of vertical distances.
    X_plot : np.ndarray, shape (n_points*n_points, 2)
        Flattened grid points (unscaled) for prediction.
    """
    d_vals = np.linspace(d_range[0], d_range[1], n_points)
    z_vals = np.linspace(z_range[0], z_range[1], n_points)
    D_grid, Z_grid = np.meshgrid(d_vals, z_vals)
    X_plot = np.column_stack((D_grid.ravel(), Z_grid.ravel()))
    return D_grid, Z_grid, X_plot


def plot_force_map(
    D_grid: np.ndarray,
    Z_grid: np.ndarray,
    force_map: np.ndarray,
    dD_train: np.ndarray,
    zD_train: np.ndarray,
    force_train: np.ndarray,
    kernel_name: str,
    save_path: str = 'RBF_Regression_V2.png'
) -> None:
    """
    Plot the predicted force contour map over the design space.

    Parameters
    ----------
    D_grid, Z_grid : np.ndarray, shape (100,100)
        Grid coordinates.
    force_map : np.ndarray, shape (100,100)
        Predicted force values on the grid.
    dD_train, zD_train : np.ndarray, shape (9,)
        Training data coordinates.
    force_train : np.ndarray, shape (9,)
        Training force values.
    kernel_name : str
        Name of the best kernel (for title).
    save_path : str
        File path to save the figure.
    """
    plt.rcParams.update({
        'font.size': 20,
        'legend.fontsize': 18,
        'axes.labelsize': 20,
        'axes.titlesize': 24,
        'xtick.labelsize': 18,
        'ytick.labelsize': 18,
        'text.usetex': False
    })

    plt.figure(figsize=(10, 8))
    plt.title(f'Best RBF Regression ({kernel_name})')
    contour = plt.contourf(D_grid, Z_grid, force_map,
                           np.linspace(-12, 0, 25), cmap='viridis_r')
    plt.scatter(dD_train, zD_train, c=force_train,
                s=250, vmin=-12, vmax=0, cmap='viridis_r',
                edgecolors='red', linewidth=2)
    plt.xlabel('Horizontal Distance $d/D$')
    plt.ylabel('Vertical Distance $z/D$')
    plt.xlim(0.20, 0.55)
    plt.ylim(0.45, 1.05)
    plt.xticks([0.25, 0.375, 0.50])
    plt.yticks([0.50, 0.75, 1.00])
    cbar = plt.colorbar(contour)
    cbar.set_label('Wing Download Force [N]')
    #plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.show()

#%% 5. MAIN WORKFLOW

def main():
    """Main workflow: load data, scale, hyperparameter search, predict, plot."""
    # 1. Load CFD data
    dD, zD, force = load_cfd_data()
    X = np.column_stack((dD, zD))                     # shape (9,2)

    # 2. Feature scaling (MinMax to [0,1])
    scaler = MinMaxScaler()
    X_scaled = scaler.fit_transform(X)                # shape (9,2)

    # 3. Define RBF centres (all training points)
    centers = X_scaled.copy()                         # shape (9,2)

    # 4. Hyperparameter ranges
    kernels = ['Gaussian', 'Matérn', 'Compact']
    sigmas = np.linspace(0.1, 3, 50)                # 25 values
    alphas = np.logspace(-4, -1, 50)                 # 25 log-spaced values

    # 5. Grid search with 3-fold CV
    (best_rmse, best_sigma, best_alpha,
     best_kernel, best_model, best_cond) = grid_search_rbf(
        X_scaled, force, centers, kernels, sigmas, alphas,
        n_folds=3, random_seed=1
    )

    # 6. Report results
    print('=' * 50)
    print('BEST MODEL')
    print('=' * 50)
    print(f'Kernel            : {best_kernel}')
    print(f'Sigma             : {best_sigma:.4f}')
    print(f'Alpha             : {best_alpha:.4e}')
    print(f'Average RMSE      : {best_rmse:.4f}')
    print(f'Condition Number  : {best_cond:.4e}')
    print('=' * 50)

    # 7. Create fine prediction grid (unscaled)
    D_grid, Z_grid, X_plot = create_prediction_grid(
        d_range=(0.25, 0.50), z_range=(0.50, 1.00), n_points=100
    )
    # Scale the prediction grid using the same scaler
    X_plot_scaled = scaler.transform(X_plot)          # shape (10000,2)

    # 8. Build design matrix for prediction
    Phi_plot = build_design_matrix(
        X_plot_scaled, centers, best_sigma, best_kernel
    )                                                # shape (10000, 9)
        
    # 9. Predict force on grid
    force_map = best_model.predict(Phi_plot)         # shape (10000,)
    force_map = force_map.reshape(D_grid.shape)      # shape (100,100) XXXXXX

    # 10. Plot and save
    plot_force_map(D_grid, Z_grid, force_map,
                   dD, zD, force, best_kernel,
                   save_path='RBF_Regression_V2.png')


if __name__ == "__main__":
    main()